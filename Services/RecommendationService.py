...
class RecommendationService:
    def processConsultant(self,consultant):
        ...
        description+=' '.join(consultant.get('listSkills'))
        return description
    def getOpportunityList(self,mysql):
        mysqlService=MySqlService()
        data= mysqlService.getAllOpportunity(mysql)
        # Convert the query result into a list of dictionaries
        table_dict_list = [{'id': item[0], 'titre': item[1], 'description': item[2],'date_deposition':item[3],'tjm':item[4],'duree':item[5],'location':item[6]} for item in data]
    ...
    def opportunityListToDataFrame(self,df1):
        columns_to_keep = ['titre', 'description', 'date_deposition','tjm','duree','location']  # Specify the columns you want to keep
    ...
    def getRecommendationOpportunity(self,mysql,consultant):
        ...
        df=self.get_recommendation('Notre',cosine_sim,indices,df1)
